...
public class HAC<T> {
    ...
    public HAC(List<T> values, DistanceFunction<T> distanceFunction) {
        this.values = values;
        this.distanceFunction = distanceFunction;
    }

    private void init(double distanceLimit) {
        heap.clear();
        triples.clear();
        communities.clear();
        idGenerator = 0;
    ...
    public ClusterizationResult<T> run(double distanceLimit, int minClustersCount) {
        init(distanceLimit);
        ...
        clearPool();
        return new ClusterizationResult<>(
                communities.stream().map(c -> c.entities).collect(Collectors.toList()),
                distanceFunction);
    }
